...
+function ($) {
	...
	var DCalendar = function(elem, options) {
		...
		this.maxDay = new Date(); // max-lange 값에 따른 선택가능 범위
		this.maxDay.setDate(this.maxDay.getDate() + maxLange);
		...
		function selectDate () {
			...
			var dayWeek = daysofweek[new Date(newDate).getDay()];
			$('.dateWeek').css('display','block');
			$('.dateWeek').text(dayWeek);
	...
	DCalendar.prototype = {
		...
		create : function(mode){
			...
			if(mode == "days"){
				...
				for(var i=1; i<=6; i++){
					...
					while(day<=days){
						...
						// 이전 일자 class 설정
						if((day < that.today.getDate() && d.getMonth() <= that.today.getMonth()) || d.getMonth() < that.today.getMonth() || d.getFullYear() < that.today.getFullYear()) {
							temp[dayOfWeek].attr('class', 'prevDay');
						}
						...
						//console.log(d.getMonth() + " == " + that.maxDay.getMonth());

						if( 
							(that.today.getMonth() == that.maxDay.getMonth() && day > that.today.getDate() && day <= that.maxDay.getDate()) ||
							(that.today.getMonth() - that.maxDay.getMonth() <= -1) &&
							((d.getFullYear() == that.today.getFullYear() && d.getMonth() == that.today.getMonth()) && day > that.today.getDate()) ||
							((d.getFullYear() == that.maxDay.getFullYear()) && (d.getMonth() > that.today.getMonth() && d.getMonth() <= that.maxDay.getMonth()) && day <= that.maxDay.getDate())
						){
							// 같은 달 이후 날짜 || day <= this.maxDay.getDate()
							// 현재 달과 최대 선택범위 달이 같다면 today ~ maxDay 까지 선택
							//(day > that.today.getDate()) && (d.getMonth() == that.today.getMonth() || d.getMonth() ==  that.maxDay.getMonth())
							// 현재 월과 최대 월이 같다면 오늘 ~ 최대일까지 범위 지정
							// 최대 월이 다음달이라면 현재 보고있는 달력의 월이 이번달이면 오늘부터 이달말까지 check d.getMonth() == that.maxDay.getMonth()
							temp[dayOfWeek].attr('class','abled')
						}
						//console.log(that.today.getDate(that.today.getMonth()+1));
						// 
					...
					if(i == 1 || i > 4){
						...
						for(var a=(i==1?6:0); (i==1?(a>=0):(a<=6)); (i==1?a--:a++)){
							if(temp[a].text() == ''){
								...
								temp[a].html('<span>'+((i==1?pDays--:nStartDate++))+'</span>').addClass((i==1?'pMDate prevDay':'nMDate'));
			...
			if(
				// nMDate 설정
				(that.today.getMonth() - that.maxDay.getMonth() == -1)
//				maxLange - (new Date(that.today.getFullYear(), that.today.getMonth()+1, 0).getDate() - that.today.getDate()) >= 1
			){

				var count = maxLange - (new Date(that.today.getFullYear(), that.today.getMonth()+1, 0).getDate() - that.today.getDate());
				if((d.getFullYear() == that.today.getFullYear() && d.getMonth() == that.today.getMonth())){
					var target = document.getElementsByClassName('nMDate');
					
					for(var i = 0; i < target.length; i++){
						if(i >= count) break;
						target[i].classList.add('abled');
					}
			...
			if(
				// pMDate 설정
				(d.getMonth() == that.maxDay.getMonth() && d.getFullYear() == that.maxDay.getFullYear())
			){
				var count = (new Date(that.today.getFullYear(), that.today.getMonth()+1, 0).getDate() - that.today.getDate());
				if((d.getFullYear() == that.maxDay.getFullYear() && d.getMonth() == that.maxDay.getMonth())){
					var target = document.getElementsByClassName('pMDate');
					console.log(target);
		...
		return $(this).each(function(){
			...
			cal.hover(function(){
				...
				if(!selectedDate){
					...
					if(e.target.parentNode.classList.contains('abled')){
						$(this).hide();
						console.log('1');
						selectedDate = false;
					}else{
